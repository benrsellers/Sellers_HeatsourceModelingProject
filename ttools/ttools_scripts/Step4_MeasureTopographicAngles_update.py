...
z_units = "Meters"
topo_fc = r"D:\Ben_wd\THESIS\heatsource\post_defense\ttools\ttools_outputs\topofc"
block_size = "#" # OPTIONAL defualt to 5
overwrite_data = True
# End Fill in Data

# Used for debugging. Currently turned off. 
topo_line_fc = r"c:\temp\topoLine.shp"
block_fc = r"c:\temp\blocks.shp"
plot_dir = r"c:\temp"
# ----------------------------------------------------------------------
                    ...
                    for i, block_segment in enumerate(block_segments):
                        ...
                        if intersects:
                            if a in [0, 180]:
                                ...
                                distance.append((inter1_y - node_y) / cos(radians(a))) #this is the distance between the node and the edge of the block
                            else:
                                distance.append((inter1_x - node_x) / sin(radians(a))) #this is the distance between the node and the end of the line
...
def get_topo_angles(nodeDict, block_extent, block_samples, z_raster, azimuthdisdict, searchDistance_max_m, con_z_to_m):
    ...
    try:
        ...
        #modded to expand array by 1px all around to ensure no out-of-bound errors (Dugdale, 2017-02-09)
        z_array = arcpy.RasterToNumPyArray(z_raster, arcpy.Point(block_x_min_center-x_cellsize, block_y_min_center-x_cellsize),
                                       ncols+2, nrows+2, nodata_to_value)
    ...
    if z_array.max() > -9999:
        ...
        for (nodeID, streamID, a, z_node,
            ...
            for distance in distance_array:
                ...
                z_topo_list.append(z_array[xy[1]+1, xy[0]+1]) #modded with the '+1' to ensure no out-of-bound errors (Dugdale, 2017-02-09)
...
#                           "appended to the existing feature class.")
    ...
    # blockIDs.sort()
    blockIDs = sorted(blockDict.keys())
