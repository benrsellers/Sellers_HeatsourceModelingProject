...
# ----------------------------------------------------------------------
# Start Fill in Data
nodes_fc = r"D:\Ben_wd\THESIS\heatsource\post_defense\ttools\ttools_outputs\nodes.shp"
trans_count = 8
transsample_count = 5 # does not include a sample at the stream node
transsample_distance = 5
heatsource8 = False
sampleID_for_code = False  # Use the sampleID instead of height for the landcover
lc_raster = r'D:\Ben_wd\THESIS\heatsource\post_defense\ttools\ttools_inputs\lc_rasteroutputs\lc_buffer'
lc_units = "Meters"
...
z_raster = r"D:\Ben_wd\THESIS\heatsource\post_defense\ttools\ttools_inputs\topo\topo6608"
z_units = "Meters"
lc_point_fc = r"D:\Ben_wd\THESIS\heatsource\post_defense\ttools\ttools_outputs\lc_point_fc"
...
def update_lc_point_fc(lc_point_list, type, lc_point_fc, nodes_fc,
    ...
    cursorfields = ["POINT_X","POINT_Y"] +["STREAM_ID","NODE_ID", "SAMPLE_ID", 
                                           "TRANS_AZM", "TRANSECT",
                                           "SAMPLE", "KEY"] + type    #TRANS_AZIMUTH shortened to TRANS_AZM to obey ArcGIS 10 char restriction. S Dugdale, 2017-02-21
    ...
    if not arcpy.Exists(lc_point_fc):
        ...
        typeDict = {"POINT_X": "DOUBLE", 
                    ...
                    "TRANSECT": "SHORT",
                    "SAMPLE": "SHORT",
                    "KEY": "TEXT",} #TRANS_AZIMUTH shortened to TRANS_AZM to obey ArcGIS 10 char restriction. S Dugdale, 2017-02-21
...
def sample_raster(block, lc_point_list, raster, con):
    ...
    x_cellsize = float(arcpy.GetRasterProperties_management(raster, "CELLSIZEX").getOutput(0)) # changed from 'z_raster' to 'raster' variable to enable rasters with different cellsizes. S Dugdale, 2017-02-21
    y_cellsize = float(arcpy.GetRasterProperties_management(raster, "CELLSIZEY").getOutput(0)) # changed from 'z_raster' to 'raster' variable to enable rasters with different cellsizes. S Dugdale, 2017-02-21
